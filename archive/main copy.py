...
from PyQt6.QtGui import QPixmap, QIcon, QFontDatabase, QPainter, QPen
...
class ImageViewer(QDialog):
    def __init__(self, image_path, face_location, parent=None):
        super().__init__(parent)
        self.image_path = image_path
        self.face_location = face_location
        self.setWindowTitle("Original Photo")

        self.pixmap = QPixmap(str(self.image_path))
        
        # Scale UI to fit the image, but cap it to screen size
        screen_geometry = QApplication.primaryScreen().availableGeometry()
        self.pixmap = self.pixmap.scaled(screen_geometry.width() - 100, 
    ...
    def paintEvent(self, event):
        """Overrides the paint event to draw a rectangle around the face."""
        super().paintEvent(event)
        painter = QPainter(self.image_label.pixmap())
        pen = QPen(Qt.GlobalColor.red, 4) # Red color, 4px thickness
        painter.setPen(pen)

        # Scale the face location coordinates to match the scaled pixmap
        original_width = QPixmap(str(self.image_path)).width()
        scale_factor = self.pixmap.width() / original_width
        
        top, right, bottom, left = self.face_location
        scaled_rect = QRect(int(left * scale_factor), 
...
class FaceTagWidget(QWidget):
    ...
    def __init__(self, portrait_path, source_image_path, face_location):
        super().__init__()
...
class FaceFolioApp(QMainWindow):
    ...
    def populate_tagging_screen(self):
        ...
        for i, face_data in enumerate(self.discovered_faces_data):
            portrait_path = face_data['portrait_path']
            source_path = face_data['source_path']
            location = face_data['location']

            tag_widget = FaceTagWidget(portrait_path, source_path, location)
            tag_widget.name_input.setText(f"Person_{i + 1}")
            tag_widget.clicked.connect(lambda sp=source_path, loc=location: self.show_full_image(sp, loc))
            
            self.tagging_layout.addWidget(tag_widget)
            self.face_tag_widgets.append((i, tag_widget))

    def show_full_image(self, image_path, face_location):
        viewer = ImageViewer(image_path, face_location, self)
        viewer.exec()
    ...
    def on_discovery_finished(self, result):
        ...
        self.discovered_faces_data = result
        print(f"--- DISCOVERY FINISHED: Found {len(self.discovered_faces_data)} unique people ---")

        if not self.discovered_faces_data:
    ...
    def run_w2_final_sort(self, progress_callback, user_names):
        core.sort_photos_by_discovered_faces(progress_callback, self.discovered_faces_data, user_names)
