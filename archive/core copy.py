...
def find_unique_faces(progress_callback, image_paths, tolerance=0.6):
    ...
    discovered_faces = [] # Will store dicts of face data
    total_images = len(image_paths)
    ...
    for i, image_path in enumerate(image_paths):
        ...
        try:
            ...
            for j, face_encoding in enumerate(face_encodings):
                ...
                matches = face_recognition.compare_faces([f['encoding'] for f in discovered_faces], face_encoding, tolerance)
                if True not in matches:
                    portrait_path = _save_portrait(image_path, face_locations[j], len(discovered_faces))
    ...
    print(f"--- Discovery complete. Found {len(discovered_faces)} unique people. ---")
    return discovered_faces

def _save_portrait(image_path, location, person_index):
    """Crops, resizes, and saves a high-quality face portrait."""
    top, right, bottom, left = location
    image = Image.open(image_path)
    face_image = image.crop((left, top, right, bottom))
    
    # Upscale to a fixed size for better quality in the UI
    upscaled_size = (256, 256)
    face_image = ImageOps.fit(face_image, upscaled_size, Image.Resampling.LANCZOS)

    portrait_path = UNKNOWN_PORTRAITS_DIR / f"person_{person_index}.png"
    face_image.save(portrait_path)
    return portrait_path

def sort_photos_by_discovered_faces(progress_callback, discovered_faces_data, user_names, tolerance=0.6):
    ...
    # Create a list of all images to process for accurate progress
    all_image_paths = set()
    for face_data in discovered_faces_data:
        all_image_paths.add(face_data['source_path'])
    
    total_images = len(all_image_paths)
    processed_count = 0

    # This is a simplified approach for demonstration. A more robust solution
    # would re-scan all photos or use a more complex data structure.
    for image_path in sorted(list(all_image_paths)):
        processed_count += 1
        progress_callback(processed_count, total_images, image_path.name)

        for face_data in discovered_faces_data:
            if face_data['source_path'] == image_path:
                match_index = discovered_faces_data.index(face_data)
